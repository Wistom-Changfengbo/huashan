...
class MobileJobUserTrack extends JobUserTrack {
  ...
  override protected def job(args: Array[String]): Unit = {
    ...
    val linesRdd = sc.textFile("G:\\LOG\\app\\clean\\")

    val beansRdd = linesRdd.map(BeanHandler.toAppAcc).filter(filterBean)
    beansRdd.repartition(1).saveAsTextFile("G:\\LOG\\app\\test01")
    //清除当天数据
//    beansRdd.foreachPartition(iterator => {
//      initHBase()
//      var tbConn = new ExTBConnection
//      var esConn = new ExEsConnection
//      tbConn = null
//      esConn = null
...
//    })
    ...
    val visitBeanRdd = beansRdd.groupBy(x => (x.appid, x.uuid, x.visitindex))
      .map(x => VisitTrackHandler.countUserVisit(x._2.toList.sortWith(_.time < _.time), pageRuleInfosBroadcast.value))
      .filter(x => x != null && x.pReqs.length + x.pEvents.length > 0)
     visitBeanRdd.repartition(1).saveAsTextFile("G:\\LOG\\app\\test02")
    //        saveVisitRdd(visitBeanRdd)
  }
}

object MobileJobUserTrack {
  def main(args: Array[String]) {
    val job = new MobileJobUserTrack()
    job.run(args)
