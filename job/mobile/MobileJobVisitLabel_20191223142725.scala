...
class MobileJobVisitLabel extends EveryDaySparkJob {
  ...
  /**
    ...
    * @return
    */
  private def pageBeans2Laebls(uuid: String, companyId: Int, beans: List[MobileLogMsg]): List[LabelDes] = {
  ...
  /**
    ...
    * @return
    */
  private def eventBeans2Laebls(uuid: String, companyId: Int, beans: List[MobileLogMsg]): List[LabelDes] = {
    val map = mutable.Map[String, (Int, Long)]()
    val labelList = new ListBuffer[LabelDes]
    val userLabelInfos = userLabelInfosBroadcast.value.getOrElse(companyId, null)
    if (userLabelInfos == null)
  ...
  /**
    ...
    * @return
    */
  private def countBaseWeight(uuid: String, companyId: Int, beans: List[MobileLogMsg]): Double = {
    ...
    val strings = beans.map(_.subtype)
    val eventSubTyes = strings.distinct
    val eventCount = {
    ...
    bean.logtype match {
      ...
      case LogType.EVENT => {
        ...
        if (bean.subtype == LogEventId.UI_CLICK_POINT
          ...
          || bean.subtype == "page_action" || bean.title.isEmpty)
          return false
        return true
      }
    }
...
//            HBaseUtils.createTable(HBaseTableName.UUID_LABEL_SUM)
    ...
    val sc = SparkExtend.ctx
    val lines = sc.textFile("G:\\LOG\\app\\clean\\")
    val logBeanRdd = lines.map(BeanHandler.toAppAcc).filter(filterBean)
    lines.map(BeanHandler.toAppAcc).filter(_.subtype=="app_guide_data").take(1).foreach(println)
    println(logBeanRdd.count())

    //按UUID和APPID分组
    val uuidAppid2BeansRdd = logBeanRdd.groupBy(x => (x.uuid, x.appid))


    val testrdd =uuidAppid2BeansRdd.map(x => {
      val uuid = x._1._1
      val appid = x._1._2
    ...
    testrdd.repartition(1).saveAsTextFile("G:\\LOG\\app\\test01")
    testrdd.reduceByKey((x1, x2) => (x1._1 + x2._1, reduceLebelDess(x1._2, x2._2))).repartition(1).saveAsTextFile("G:\\LOG\\app\\test02")
//    val tuples: Array[((String, Int), (Double, List[MobileJobVisitLabel.this.LabelDes]))]= testrdd.take(10)
...
object MobileJobVisitLabel {

  def main(args: Array[String]) {
    val job = new MobileJobVisitLabel()
    job.run(args)
